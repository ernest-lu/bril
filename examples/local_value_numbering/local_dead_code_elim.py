...
def process_fn(fn_instrs):
    blocks = form_blocks(fn_instrs)
    blocks_map = block_map(blocks)

    for name in blocks_map:
        while True:
            new_block = process_block(blocks_map[name])
            assert len(new_block) <= len(blocks_map[name])
            if len(new_block) == len(blocks_map[name]):
                break
            blocks_map[name] = new_block
...
if __name__ == "__main__":
    ...
    for fn in prog["functions"]:
        fn["instrs"] = process_fn(fn["instrs"])
